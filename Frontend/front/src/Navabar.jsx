...
function Navabar({ hide }) {
  const { filterFun, getUser, setCurrHotelDetail, hotelData } = useContext(AppContext)
  const [getSearchQuery, serSearchQuery] = useState(null)
  const getSearch = useDebounce(getSearchQuery)
  const [focusActive, focusUnActive] = useState(false)
  const navigate = useNavigate()

  const { isError, data, isLoading } = useQuery({
    queryKey: ["search", getSearch],
    queryFn: async () => {
      const data = await axios.get(`${import.meta.env.VITE_SEARCH}?query=${getSearch}`)
      console.log(data.data.mydata)

      return data.data?.mydata || false
    },
    enabled: !!getSearch
  })
  ...
  useEffect(() => {
    // console.log("run Fase ", getSearch)
    // filterFun(getSearch)
    if (data) {
      setCurrHotelDetail(data)
    }
    else {
  ...
  if (isLoading) {
    console.log("loading ho rah ahi ....")
  }
  if (isError) {
    console.log(isError)
  }
      ...
      <div className="container-fluid">
        <a className="navbar-brand" href="/myprofile"><div style={{ height: '50px', width: '50px' }}><img src={
          getUser.profile
            ...
            : user1} className="img-fluid  rounded-circle" alt="..." style={{
              height: '100%',
              width: '100%',
              objectFit: "cover",
        ...
        <div className="collapse navbar-collapse" id="navbarScroll">
          ...
          </ul>
          {
            hide
              ...
              : <form className="d-flex border align-items-center pe-2  col-lg-8 ms-auto" style={{ borderRadius: "10px" }}>
                <input
                  ...
                  className="form-control me-2"  placeholder="City name...." onChange={(e) => {
                    serSearchQuery(e.target.value)
                    if (!focusActive) {
                      focusUnActive(true)
                    }
                  }} style={{ border: "none" }} value={getSearchQuery} />
                {
                  focusActive && <LoadingCom value={true} />
